...
- **GitHub Pages 미러**: https://anomixer.github.io/js7800/
...
*   **글로벌 리더보드 다국어 지원**: 글로벌 리더보드 페이지도 동일한 11개 언어를 지원하며 메인 에뮬레이터의 언어 설정과 자동으로 동기화됩니다.
...
### 08/28/26 (0.1.0 - 포크 버전)
    - 다국어 지원 (11개 언어: 한국어, 영어, 번체 중국어, 간체 중국어, 일본어, 독일어, 스페인어, 프랑스어, 이탈리아어, 포르투갈어, 러시아어)
    - 글로벌 하이스코어 리더보드 실시간 동기화 및 Master SRAM 보호
    - Cloudflare Pages 및 GitHub Pages 미러 듀얼 자동 배포
    - 레트로 HighScore Cartridge 하드웨어 호환 2단계 SRAM 쓰기 지원
    - 8병렬 초고속 동기화 및 Smart Diff 무료 캐시 최적화
    - 이벤트 기반 동기화: 공식 업스트림 0초 즉시 저장 및 클라우드 워크플로우를 통한 약 18초 초고속 미러 동기화
